...
class DriverTrackingEngine {
  ...
  private pathBuffer: {lat: number, lng: number}[] = [];
  ...
  public activeType: 'bus' | 'ferry' = 'bus';
  ...
  // Configuration
  private readonly MIN_UPDATE_INTERVAL_MS = 2500; 
  private readonly MIN_DISTANCE_METERS = 10; 
  private readonly MIN_SPEED_CHANGE_KMH = 5; 
  private readonly MIN_HEADING_CHANGE_DEG = 20; 
  private readonly STATIONARY_UPDATE_INTERVAL_MS = 15000;
  ...
  public async start(busId: string, routeName: string, routeNumber: string, type: 'bus' | 'ferry' = 'bus') {
    if (this.isTracking) {
    ...
    this.watchId = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        if (needsUpdate) {
          ...
          this.pathBuffer.push({ lat: latitude, lng: longitude });
          if (this.pathBuffer.length > 300) {
            this.pathBuffer = this.pathBuffer.slice(-300); // Keep last 300 points
          }
...
export const startDriverTracking = (busId: string, routeName: string, routeNumber: string, type: 'bus' | 'ferry' = 'bus') => trackingEngine.start(busId, routeName, routeNumber, type);

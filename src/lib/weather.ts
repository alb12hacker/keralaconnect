  ...
  useEffect(() => {
    async function fetchWeather() {
      try {
        ...
        if (code >= 61 && code <= 65) alert = 'Rain expected. Expect minor delays due to wet roads.';
        if (code >= 80 && code <= 82) alert = 'Heavy rain showers. Waterlogging possible. Plan for delays.';
        if (code >= 95 && code <= 99) alert = 'Thunderstorms reported. Expect major transit delays and reroutes.';
